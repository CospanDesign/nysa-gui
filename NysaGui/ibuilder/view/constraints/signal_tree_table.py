...
class IndexSignalLeafNode(LeafNode):
    def __init__(self, signal_name, signal_index, direction, active = True):
...
class SignalLeafNode(LeafNode):
    ...
    def __init__(self, signal_name, signal_range, direction, parent = None, used_list = []):
        ...
        if self.has_range():
            for i in range (min(signal_range), max(signal_range) + 1):
                ...
                isln = IndexSignalLeafNode(self.signal_name, i, self.direction, active)
                isln.parent = self
...
class ModuleBranch(BranchNode):
    ...
    def get_signal_name(self):
        return self.children[0][NODE].signal_name
...
class SignalTreeTableModel(QAbstractItemModel):
    ...
    def addRecord(self, color, module_name, signal_name, signal_range, direction, callReset=True, used_list = []):
        #print "TT: add record"
        ...
        if module_branch is None:
            ...
            module_branch = ModuleBranch(color, QString(module_name))
    ...
    def data(self, index, role):
        ...
        if isinstance(node, ModuleBranch):
            ...
            if index.column() == 1:
                return QString(node.get_signal_name())
                pass
