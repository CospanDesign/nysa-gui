...
class I2CController(object):
    ...
    def remove_transaction(self, loop):
        self.status.Important( "Removing a transaction")
        if not loop:
    ...
    def add_token(self, loop, row, column):
        ...
        if not loop:
            self.status.Debug( "Init: Adding a token after: %d, %d" % (row, column))
            transaction = self.init_tokens[row]
            #Got the transaction
            ...
            self.status.Debug( "Loop: Adding a token after: %d, %d" % (row, column))
            transaction = self.loop_tokens[row]
            if transaction.is_reading():
                #Reading
    ...
    def remove_token(self, loop, row, column):
        transaction = None
        if not loop:
            self.status.Debug( "Loop: Removing a token after: %d, %d" % (row, column))
            transaction = self.init_tokens[row]
        else:
            self.status.Debug( "Init: Removing a token after: %d, %d" % (row, column))
            transaction = self.loop_tokens[row]
    ...
    def row_reading_changed(self, row, enable, loop):
        self.status.Debug( "RW Changed: %d" % row)
        if not loop:
